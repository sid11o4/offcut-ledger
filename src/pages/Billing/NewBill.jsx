...
export default function NewBill() {
  ...
  const effectiveTax = taxPercent === '' ? Number(settingsQ.data?.default_tax_percent || 0) : Number(taxPercent)
  const taxAmount = Math.round(subtotal * effectiveTax) / 100
  const grandTotal = subtotal + taxAmount + (Number(adjustments) || 0)
  ...
  async function generate() {
    ...
    const { data, error } = await supabase.rpc('generate_bill', {
      ...
      p_tax_percent: effectiveTax, p_adjustments: Number(adjustments) || 0,
        ...
        <Card title="Bill Summary">
          <div className="grid md:grid-cols-2 gap-5">
            <div className="space-y-3">
              <div><label className="field-label">Tax % <span className="text-ink-400 font-normal">(default {settingsQ.data?.default_tax_percent ?? 0}%)</span></label><input className="field-input" type="number" step="0.01" placeholder={String(settingsQ.data?.default_tax_percent ?? 0)} value={taxPercent} onChange={(e) => setTaxPercent(e.target.value)} /></div>
            ...
            <div className="space-y-1 text-sm">
              ...
              <div className="flex justify-between"><span>Tax</span><span>{money(taxAmount)}</span></div>
